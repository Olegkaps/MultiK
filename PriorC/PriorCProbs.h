...
float** fit_Spline(var* mainDic, float* x_, float* y_, var passNo, numbers nums)
{
    ...
    x[0] = x_[0];
    x[n] = x_[nums.noOfBins-1];
    a[0] = y_[0];
    a[n] = y_[nums.noOfBins-1];
    for(var i = 1; i < n; i++) {
        x[i] = x_sum / k;
        a[i] = y_sum / k;

        x_sum = x_sum - x_[i] + x_[i + k];
        y_sum = y_sum - y_[i] + y_[i + k];
    }
    ...
    float t;
    for(var i = 0; i < nums.noOfBins; i++) {
        if(ind < n && x_[i] > x[ind+1]) {
            ind += 1;
        }
        t = x_[i] - x[ind];
        y_[i] = a[ind] + b[ind]*t + c[ind]*t*t + d[ind]*t*t*t;
    }*/
