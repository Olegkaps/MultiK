...
void showHelp(void)
{
                ...
                fprintf(stdout, "-d\t--dna_contact_density=FileName\tOPTIONAL: compute density of dna contacts\n\n");
...
priorCArgs* ProcessArgs(priorCArgs* args) {
    ...
    if ((strcmp(args->dnaDensityFileName, ""))) {
        args->density_table = xmalloc(sizeof(DNA_chr_bin_hash_table));
        args->density_table = init_hash_table(args->fragsfileName, args->binLength);
    }
...
priorCArgs* parse(priorCArgs* args, int argc, char *argv[]) {
    ...
    static struct option long_options[] =
    {
        ...
        {"dna_contact_density", required_argument, NULL, 'd'},
        {"help", no_argument, NULL, 'h'},
        {NULL, 0, NULL, 0}
    };
    
    while ((c = getopt_long(argc, argv, ":i:f:r:o:p:b:l:U:L:d:h", long_options, NULL)) != -1)
    {
        switch (c)
        {
            case 'i':
            ...
            case 'L':
                ...
                break;
            case 'd':
                fprintf(stdout, "Density file would be %s\n", optarg);
                args->dnaDensityFileName = optarg;
                break;
            case 'h':
                showHelp();
                exit(0);
